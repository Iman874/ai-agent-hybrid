# Task 10 — Integration Test: DecisionEngine Routing Scenarios

## 1. Judul Task

Buat integration tests untuk `DecisionEngine` yang menguji keseluruhan flow routing: chat → escalation → generate, dengan SQLite database aktif.

## 2. Deskripsi

Integration test yang menguji DecisionEngine secara end-to-end menggunakan database SQLite nyata (tmp_path). Test menggunakan mock GeminiProvider dan OllamaProvider jika API keys tidak tersedia, atau skip jika infrastructure tidak ready.

## 3. Tujuan Teknis

- `tests/test_decision_engine.py` — 5+ integration tests
- Test scenarios: new session, completed session, escalation via rules, force generate
...
### Step 1: Buat `tests/test_decision_engine.py`
...
```bash
.\venv\Scripts\pytest.exe tests/test_decision_engine.py -v
```

## 6. Output yang Diharapkan

```
tests/test_decision_engine.py::test_escalation_checker_with_real_session PASSED
tests/test_decision_engine.py::test_escalation_logger_persists PASSED
tests/test_decision_engine.py::test_progress_tracker_stagnation_scenario PASSED
tests/test_decision_engine.py::test_completed_session_returns_info PASSED
====================== 4+ passed ======================
```

## 7. Dependencies

- **Semua task 01-08** harus selesai

## 8. Acceptance Criteria

- [ ] `pytest tests/test_decision_engine.py -v` → 4+ tests PASSED
- [ ] Escalation rules bekerja dengan session dari real DB
- [ ] EscalationLogger persist ke SQLite
- [ ] ProgressTracker + EscalationChecker bekerja terintegrasi
- [ ] Completed session handling benar
